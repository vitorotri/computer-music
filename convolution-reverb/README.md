...
The MAIN.pd file has the main patch. Follow the instructions inside the patch to read an audio file and do the convolution using the scene.txt file. It has the built-in option to read 2 anechoic audio files inside the same directory: laughing.wav and singing.wav. If desired, change the name to your wav file in the message box. The scene.txt file contains the configuration for the room in the following format:

<pre style="font-family:'Courier New', monospace; font-size:smaller;">
Lx Ly Lz
